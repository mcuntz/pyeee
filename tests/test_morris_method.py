...
python -m pytest --cov=. --cov-report term-missing -v tests/test_morris_method.py
...
class TestMorrisMethod(unittest.TestCase):
    """
    Tests for morris_method.py
    Missing coverage:
        ca. 405ff: exactly same trajectory sampled by chance
        ca. 500-550: ImportError if matplotlib not installed
        ca. 730: only one trajectory
    ...
    def test_r_10_diag(self):
        ...
        # Check 1
        mat, vec = morris_sampling(self.nparam, self.LB, self.UB, nt,
                                   nsteps=nsteps, ntotal=ntotal, Diagnostic=1)
        self.assertEqual(list(np.around(mat[0, 0:5], 3)),
                         [0.6, 2.2, 2., 4.6, 8.])
        self.assertEqual(list(np.around(vec[0:5], 3)),
                         [12., 11., 5., 1., 9.])
        ...
        if os.path.exists('morris_diag_new_strategy.png'):
            os.remove('morris_diag_new_strategy.png')
        if os.path.exists('morris_diag_old_strategy.png'):
            os.remove('morris_diag_old_strategy.png')
    ...
    def test_r_10_nan(self):
        ...
        sa, res = elementary_effects(self.nparam, mat, vec, out, nsteps=nsteps)
        self.assertEqual(list(np.around(res[0:5, 0], 3)),
                         [0.368, 0.309, 0.549, 0.534, 0.65])
        self.assertEqual(list(np.around(sa[~np.isnan(sa[:, 1]), 1], 3)),
                         [0.47, 0.816, 0.722, 0.418, -0.653, -0.941, 0.863,
                          -1.265, -0.424, -0.786, 0.183])
    ...
    def test_r_1(self):
        ...
        sa, res = elementary_effects(self.nparam, mat, vec, out, nsteps=nsteps)
        self.assertEqual(list(np.around(res[0:5, 0], 3)),
                         [0.579, 0.009, 0.239, 0.864, 0.876])
        self.assertEqual(list(np.around(sa[0:5].squeeze(), 3)),
                         [-0.579, -0.009, -0.239, -0.864, 0.876])
    ...
    def test_groups_nan(self):
        ...
        mat, vec = morris_sampling(self.nparam, self.LB, self.UB, nt,
                                   nsteps=nsteps, ntotal=ntotal,
                                   GroupMat=Groups, Diagnostic=self.Diagnostic)
        sa, res = elementary_effects(self.nparam, mat, vec, out,
                                     nsteps=nsteps, Group=Groups)
        self.assertEqual(list(np.around(res[0:5, 0], 3)),
                         [0.49, 0.425, 0.427, 0.441, 0.441])
        self.assertEqual(list(np.around(sa[0:5, 1], 3)),
                         [0.279, 0.557, 0.557, 0.557, 0.557])
    ...
    def test_r_10_dist(self):
        ...
        # Check 1
        mat, vec = morris_sampling(self.nparam, LB, UB, nt, nsteps=nsteps,
    ...
    def test_r_10_dist_none(self):
        ...
        # Check 3
        np.random.seed(seed=self.seed)
        mat1, vec1 = morris_sampling(self.nparam, LB, UB, nt, nsteps=nsteps,
                                     ntotal=ntotal, dist=dist,
                                     distparam=distparam1,
                                     Diagnostic=self.Diagnostic)
        np.random.seed(seed=self.seed)
        mat2, vec2 = morris_sampling(self.nparam, LB, UB, nt, nsteps=nsteps,
                                     ntotal=ntotal, dist=dist,
    ...
    def test_r_10_dist_raise(self):
        import numpy as np
        import scipy.stats as stats
        from pyeee import morris_sampling
        ...
        # Check 1
        self.assertRaises(AssertionError, morris_sampling, self.nparam, LB[1:],
                          UB, nt, nsteps=nsteps, ntotal=ntotal,
                          Diagnostic=self.Diagnostic)

        # Check 2
        self.assertRaises(AssertionError, morris_sampling, self.nparam, LB,
                          UB[1:], nt, nsteps=nsteps, ntotal=ntotal,
                          Diagnostic=self.Diagnostic)

        # Check 3
        self.assertRaises(AssertionError, morris_sampling, self.nparam, LB, UB,
                          nt, nsteps=nsteps, ntotal=ntotal, dist=dist[1:],
                          distparam=distparam, Diagnostic=self.Diagnostic)
